...
const pizzaYardEdgeClient=makePizzaYardEdgeClient(supabaseClient);
...
const BREAKFAST_MENU=[
 ...
 {id:"cocoa_tea",name:"Cocoa Tea",price:3},
 {id:"local_juice",name:"Local Juice",price:6}
];
const breakfastQty=Object.fromEntries(BREAKFAST_MENU.map(x=>[x.id,0]));
const staffDrinkPrices={cocoa_tea:3,local_juice:6};
const state={category:"pizza",mode:"whole",whole:[],left:[],right:[],quantity:1,type:"pickup",availability:Object.fromEntries(TOPPINGS.map(t=>[t,true])),repeatOrder:null,cart:[],paymentStatus:"unpaid",paymentMethod:"",storeMode:false,drinks:{cocoa_tea:0,local_juice:0}};
...
const FAST_PRESETS={pepperoni:["Pepperoni"],"pep-mush":["Pepperoni","Mushroom"],meat:["Pepperoni","Bacon","Ham","Sausage"],veg:["Bell Peppers","Mushroom","Corn","Veg"]};
function applyFastPreset(id){const picks=(FAST_PRESETS[id]||[]).filter(t=>state.availability[t]!==false);if(!picks.length)return;state.mode="whole";state.whole=picks;state.left=[];state.right=[];$$('.mode[data-mode]').forEach(b=>b.classList.toggle('active',b.dataset.mode==='whole'));$('#whole-builder')?.classList.remove('hidden');$('#half-builder')?.classList.add('hidden');renderToppings();updateSummary();document.querySelector('.fast-pick.active')?.classList.remove('active');document.querySelector(`.fast-pick[data-preset="${id}"]`)?.classList.add('active');}
...
async function loadDrinkPrices(){try{const {data,error}=await supabaseClient.from('menu_items').select('id,price,available').eq('category','Drinks');if(error||!data)return;data.forEach(x=>{if(x.id in staffDrinkPrices){staffDrinkPrices[x.id]=Number(x.price||0);}});const c=document.querySelector('#drink-cocoa')?.closest('label'),j=document.querySelector('#drink-juice')?.closest('label');if(c)c.firstChild.textContent=`☕ Cocoa Tea `;if(j)j.firstChild.textContent=`🥤 Local Juice `;const spans=document.querySelectorAll('.staff-drink-grid label span');if(spans[0])spans[0].textContent=`$${staffDrinkPrices.cocoa_tea.toFixed(2)}`;if(spans[1])spans[1].textContent=`$${staffDrinkPrices.local_juice.toFixed(2)}`;updateSummary()}catch(e){console.warn(e)}}
async function loadAvailability(){try{const{data,error}=await pizzaYardEdgeClient.from("pizza_topping_availability").select("name,available");if(!error&&data)data.forEach(r=>state.availability[r.name]=r.available!==false)}catch(_){}renderToppings();updateSummary()}
function showApp(session){$("#login-view").classList.toggle("hidden",!!session);$("#app-view").classList.toggle("hidden",!session);if(session){loadAvailability();loadDrinkPrices()}}
...
async function searchCustomers(term){const box=$("#customer-suggestions");if(!term){box.classList.add("hidden");box.innerHTML="";return}const{data,error}=await pizzaYardEdgeClient.from("pizza_orders").select("customer_name,customer_phone").or(`customer_name.ilike.%${term}%,customer_phone.ilike.%${term}%`).order("created_at",{ascending:false}).limit(8);if(error||!data){box.classList.add("hidden");return}const seen=new Set(),rows=data.filter(x=>{const k=`${x.customer_name||""}|${x.customer_phone||""}`;if(seen.has(k))return false;seen.add(k);return true});box.innerHTML=rows.length?rows.map((x,i)=>`<button type="button" class="customer-suggestion" data-i="${i}"><span><strong>${esc(x.customer_name||"Walk-in Customer")}</strong><span>${esc(x.customer_phone||"No phone")}</span></span><b>USE</b></button>`).join(""):'<div class="customer-suggestion"><span>No previous customer found.</span></div>';box.classList.remove("hidden");$$('.customer-suggestion[data-i]').forEach(b=>b.addEventListener("click",()=>{const x=rows[Number(b.dataset.i)];$("#customer-name").value=x.customer_name||"";$("#customer-phone").value=x.customer_phone||"";box.classList.add("hidden");$("#customer-search").value="";loadRepeatOrder(x.customer_phone||"")}))}
async function loadRepeatOrder(phone){state.repeatOrder=null;$("#repeat-last")?.classList.add("hidden");if(!phone)return;const{data,error}=await pizzaYardEdgeClient.from("pizza_orders").select("*").eq("customer_phone",phone).neq("status","cancelled").order("created_at",{ascending:false}).limit(1).maybeSingle();if(error||!data)return;state.repeatOrder=data;const b=$("#repeat-last");if(b){b.textContent=`🔁 REPEAT LAST ORDER ${data.order_number?"#"+data.order_number:""}`;b.classList.remove("hidden")}}
...
function resetForm(){BREAKFAST_MENU.forEach(x=>breakfastQty[x.id]=0);state.category="pizza";state.mode="whole";state.whole=[];state.left=[];state.right=[];state.quantity=1;state.type="pickup";state.cart=[];state.paymentStatus="unpaid";state.paymentMethod="";state.drinks={cocoa_tea:0,local_juice:0};$("#staff-order-form").reset();$("#qty").textContent="1";$("#customer-search").value="";$("#repeat-last")?.classList.add("hidden");$("#delivery-fields").classList.add("hidden");$$('.mode[data-mode]').forEach(b=>b.classList.toggle('active',b.dataset.mode==="whole"));$$('.mode[data-type]').forEach(b=>b.classList.toggle('active',b.dataset.type==="pickup"));$$('.payment').forEach(b=>b.classList.toggle('active',b.dataset.payment==="unpaid"));$("#payment-method").value="";$("#submit-error").textContent="";$("#delivery-error").textContent="";renderBreakfastMenu();renderToppings();setCategory("pizza");updateSummary()}
function nextSunday(){const d=new Date();const day=d.getDay();d.setDate(d.getDate()+(7-day)%7);return d.toISOString().slice(0,10)}
async function submit(e){e.preventDefault();$("#submit-error").textContent="";$("#delivery-error").textContent="";const name=$("#customer-name").value.trim(),phone=normalizePhone($("#customer-phone").value),email=$("#customer-email").value.trim(),address=$("#address").value.trim(),birthday=$("#customer-birthday")?.value||"";if(!name)return $("#submit-error").textContent="Customer name is required.";if(!phone)return $("#submit-error").textContent="Customer phone number is required.";if(!validPhone(phone))return $("#submit-error").textContent="Enter a valid 7-digit Saint Lucia phone number.";if(state.category==="breakfast"){const selected=selectedBreakfastItems();if(!selected.length)return $("#submit-error").textContent="Choose at least one breakfast item.";const total=breakfastTotal();const b=$("#send-order");b.disabled=true;b.textContent="SENDING…";const target=nextSunday();const payload={target_sunday:target,customer_name:name||"Walk-in Customer",customer_phone:phone||"",items:selected,total,special_instructions:$("#instructions").value.trim()||null,customer_birthday:birthday||null,status:"new"};const{error}=await pizzaYardEdgeClient.from("breakfast_orders").insert(payload);b.disabled=false;b.textContent="SEND ORDER TO DASHBOARD →";if(error){console.error(error);$("#submit-error").textContent="Could not send the breakfast order.";return}$("#success-text").textContent=`Breakfast order is now on the live dashboard.`;$("#success").classList.remove("hidden");resetForm();return}const all=[...state.cart,currentItem()],totalBoxes=all.reduce((s,x)=>s+Number(x.quantity||1),0);if(phone&&!validPhone(phone))return $("#submit-error").textContent="Enter a valid 7-digit Saint Lucia phone number.";if(state.mode==="half"&&(!state.left.length||!state.right.length))return $("#submit-error").textContent="Choose at least one topping on each half.";if(state.type==="delivery"&&totalBoxes<3)return $("#submit-error").textContent="Delivery requires 3 boxes or more.";if(state.type==="delivery"&&!address)return $("#delivery-error").textContent="Delivery address is required.";const delivery=state.type==="delivery"?5:0,drinkTotal=Number($("#drink-cocoa")?.value||0)*staffDrinkPrices.cocoa_tea+Number($("#drink-juice")?.value||0)*staffDrinkPrices.local_juice,subtotal=all.reduce((s,x)=>s+Number(x.unit_price||0)*Number(x.quantity||1),0),total=subtotal+delivery+drinkTotal,items=all.map((x,i)=>({...x,item_number:i+1}));const first=items[0],token=Array.from(crypto.getRandomValues(new Uint8Array(18)),b=>b.toString(16).padStart(2,"0")).join("");const payload={customer_name:name||"Walk-in Customer",customer_phone:phone||"",customer_email:email||null,order_type:state.type,delivery_address:address||null,pizza_size:'12"',toppings:first.toppings,topping_count:first.topping_count,unit_price:first.unit_price,included_toppings:Math.min(first.topping_count,2),extra_toppings:first.extra_toppings,extra_topping_cost:first.extra_topping_cost,quantity:totalBoxes,delivery_fee:delivery,special_instructions:$("#instructions").value.trim()||null,total,status:"new",tracking_token:token,order_items:items,payment_status:state.paymentStatus,payment_method:$("#payment-method").value||null,customer_birthday:birthday||null,order_details:JSON.stringify({pizzas:items,extras:{cocoa_tea:Number($("#drink-cocoa")?.value||0),local_juice:Number($("#drink-juice")?.value||0)},drink_prices:staffDrinkPrices})};const b=$("#send-order");b.disabled=true;b.textContent="SENDING…";const{data,error}=await pizzaYardEdgeClient.from("pizza_orders").insert(payload).select("order_number,id").single();b.disabled=false;b.textContent="SEND ORDER TO DASHBOARD →";if(error){console.error(error);$("#submit-error").textContent=error.message.includes("order_items")?"Please run the new Pizza Yard v2 SQL in Supabase first.":"Could not send the order. Check the dashboard/Supabase connection.";return}$("#success-text").textContent=`Order ${data?.order_number?"#"+data.order_number:""} with ${items.length} different pizza${items.length===1?"":"s"} is now on the live dashboard.`;$("#success").classList.remove("hidden");resetForm()}
function init(){
 ...
 $$(".category").forEach(b=>b.addEventListener("click",()=>setCategory(b.dataset.category)));$("#login-form").addEventListener("submit",login);$("#logout").addEventListener("click",()=>supabaseClient.auth.signOut());$("#staff-order-form").addEventListener("submit",submit);$("#new-order").addEventListener("click",()=>$("#success").classList.add("hidden"));$("#add-pizza").addEventListener("click",addCurrentToCart);$("#fast-picks").querySelectorAll(".fast-pick").forEach(b=>b.addEventListener("click",()=>applyFastPreset(b.dataset.preset))); $("#store-mode").addEventListener("click",async()=>{state.storeMode=!state.storeMode;document.body.classList.toggle("store-mode",state.storeMode);$("#store-mode").textContent=state.storeMode?"📱 EXIT TABLET MODE":"📱 TABLET MODE";localStorage.setItem("pizzaYardStoreMode",state.storeMode?"1":"0");if(state.storeMode&&document.documentElement.requestFullscreen){try{await document.documentElement.requestFullscreen()}catch(_){}}if(!state.storeMode&&document.fullscreenElement&&document.exitFullscreen){try{await document.exitFullscreen()}catch(_){}}});
