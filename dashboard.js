...
function repeatOrder(o){
  ...
  pizzaYardEdgeClient.from("pizza_orders").insert(payload).select("order_number,id").single().then(({data,error})=>{if(error){console.error(error);showToast("Could not repeat this order.");return}showToast(`Repeat order ${data?.order_number?"#"+data.order_number:"created"}.`)});
}
...
async function loadOrders(){if(!supabaseClient){$("#orders-list").innerHTML='<div class="empty-state">Supabase is not configured.</div>';return}const{data,error}=await pizzaYardEdgeClient.from("pizza_orders").select("*").order("created_at",{ascending:false});if(error){console.error(error);$("#orders-list").innerHTML='<div class="empty-state">Unable to load orders. Check staff permissions.</div>';return}state.orders=data||[];updateStats();renderReports();renderOrders();renderDetails()}
...
let pizzaOrdersPollTimer=null;
function subscribeToOrders(){
  if(pizzaOrdersPollTimer) clearInterval(pizzaOrdersPollTimer);
  setConnection(true);
...
function showDashboard(session){$("#login-view").classList.toggle("hidden",!!session);$("#dashboard-view").classList.toggle("hidden",!session);if(session){subscribeToOrders();loadOrders();loadAvailability();loadReviews()}else { if(pizzaOrdersPollTimer)clearInterval(pizzaOrdersPollTimer); pizzaOrdersPollTimer=null; if(breakfastPollTimer)clearInterval(breakfastPollTimer); breakfastPollTimer=null; }}
async function loadReviews(){const el=$("#reviews-admin-list");if(!el)return;const{data,error}=await pizzaYardEdgeClient.from("pizza_reviews").select("*").order("created_at",{ascending:false});if(error){el.innerHTML='<span class="muted">Reviews database setup is required before reviews can be managed.</span>';return}state.reviews=data||[];renderReviews()}
...
async function updateReview(id,approved){const{error}=await pizzaYardEdgeClient.from("pizza_reviews").update({approved}).eq("id",id);if(error){showToast("Could not update review.");return}const r=state.reviews.find(x=>x.id===id);if(r)r.approved=approved;renderReviews();showToast(approved?"Review approved.":"Review hidden.")}
