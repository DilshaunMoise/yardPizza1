...
async function loadBackInStockRequests(){const el=$('#back-in-stock-list');if(!el||!sb)return;const {data,error}=await pizzaYardEdgeClient.from('back_in_stock_requests').select('*').order('created_at',{ascending:false}).limit(100);if(error){el.innerHTML='<div class="muted">Back-in-stock requests unavailable.</div>';return}const rows=data||[];el.innerHTML=rows.length?rows.map(r=>`<div class="history-row"><strong>${esc(r.topping_name)}</strong><span>${esc(r.customer_email||'No email')}</span><span>${esc(r.customer_phone||'No phone')}</span><small>${new Date(r.created_at).toLocaleString()}${r.notified_at?' • Notified':''}</small>${!r.notified_at?`<button class="ghost" data-notify-request="${r.id}">Mark Notified</button>`:''}</div>`).join(''):'<div class="muted">No customer requests yet.</div>';el.querySelectorAll('[data-notify-request]').forEach(b=>b.addEventListener('click',async()=>{const {error}=await pizzaYardEdgeClient.from('back_in_stock_requests').update({notified_at:new Date().toISOString()}).eq('id',b.dataset.notifyRequest);if(!error)loadBackInStockRequests()}))}
document.addEventListener('DOMContentLoaded',()=>$('#refresh-requests')?.addEventListener('click',loadBackInStockRequests));


async function loadDrinkSettings(){const el=$('#drink-price-list');if(!el||!sb)return;const {data,error}=await pizzaYardEdgeClient.from('menu_items').select('id,name,category,price,available').eq('category','Drinks').order('name');if(error){el.innerHTML='<div class="muted">Drink settings unavailable.</div>';return}el.innerHTML=(data||[]).map(x=>`<div class="drink-price-row"><strong>${esc(x.name)}</strong><label>Price <input type="number" min="0" step="0.01" value="${Number(x.price||0).toFixed(2)}" data-drink-price="${esc(x.id)}"></label><label class="check"><input type="checkbox" ${x.available!==false?'checked':''} data-drink-available="${esc(x.id)}"> Available</label><button class="primary" data-save-drink="${esc(x.id)}">Save</button></div>`).join('')||'<div class="muted">No drinks configured.</div>';el.querySelectorAll('[data-save-drink]').forEach(b=>b.addEventListener('click',async()=>{const id=b.dataset.saveDrink;const price=Number(el.querySelector(`[data-drink-price="${id}"]`).value||0);const available=el.querySelector(`[data-drink-available="${id}"]`).checked;const {error}=await pizzaYardEdgeClient.from('menu_items').update({price,available,updated_at:new Date().toISOString()}).eq('id',id);if(error){alert('Could not save drink settings.');return}loadDrinkSettings()}))}
const oldLoadAllPY2=loadAll;loadAll=async function(){await oldLoadAllPY2();await loadDrinkSettings();await loadBackInStockRequests()};
